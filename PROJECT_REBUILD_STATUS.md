...
| R. Slice B 爆款/共题/选题 | 进行中 | Task 8–10 已完成：Hotness、规则垂类分类、独立博主/跨平台扩散和证据型选题已串联。四垂类与 `short-video`/`tool-review`/`news-commentary`/`deep-dive`/`general` 五类画像都输出具体对象、why-now、受众、形式、钩子、提纲、原帖来源、不确定性与披露风险。单一来源不得写成趋势，抽象人文问题直接拒绝；Creator Topic 可通过 ContentService 生成原帖证据包，旧 Signal brief 与 `tool-review` 保持兼容。Task 10 定向 `5/5`，Content 联动 `11/11`。Task 11 待开始。 |
