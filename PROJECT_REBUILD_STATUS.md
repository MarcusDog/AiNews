...
| S. Slice C 持久推送与产品页面 | 进行中 | Task 12 已完成：订阅、投递端点、事件与 outbox 持久化落地；采集、Hotness 与 Creator Topic 的通知型状态变化统一经 `applyCreatorStateChange`，状态/event/outbox 同事务提交。支持垂类、平台、博主、事件类型、最低分、即时/日报与可选静默时段；未配置静默时段不会延迟实时热点。Outbox 支持 2xx 确认、429 `Retry-After`、5xx/网络指数退避、4xx 死信、崩溃 lease 恢复与手动重放。登录用户可管理自己的规则和端点，secret reference 不回显。Task 12 定向 `31/31`，服务端全量 `289/289`。Task 13 待开始。 |
