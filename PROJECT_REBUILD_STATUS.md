...
| U. 真实来源 Canary、浏览器验收与 GitHub 更新 | 已完成 | 11 账号首轮 358 条、重放新增 0、三页回填后 526 条；100/100 条 HTTPS 证据结构合格、20/20 原链可打开。Server `327/327`、Client `48/48`、Skill `17/17`，10 万帖子最慢 P95 `116.33ms` 通过。AiNews 交付提交 `664a67bc335e91a346fd54f5daf3619bed6e06ef` 与 AyaNewsSkill `1de607a9a95274fb9445d5a25e0256f932676de0` 已推送到两个现有 PR 分支，远端 SHA 完全一致。详见 `docs/verification/2026-08-29-creator-intelligence-final-verification.md`。 |
| V. 完整系统使用方案 | 已完成 | 新增 `docs/SYSTEM_USAGE_GUIDE.md`，统一覆盖四类用户、全部产品页面、每日内容工作流、Signal/Creator 来源分级、观察名单与回填、Sidecar、评分边界、推送、开放 API、数据维护、Canary、生产部署和验收；README、Quickstart、Deploy 已建立入口并修正文档中的过期端口、环境变量与不安全数据库排障说明。历史测试凭据已从当前文档移除。 |
...
- 2026-08-31：按用户要求完成 Task 10 全量离线归档。此前 954 KB 包是主动排除数据库、依赖、环境与运行状态的安全源码包；`AyaNews-full-offline-20260831.tar.gz` 只排除 `.git` 元数据，包含前后端/API 源码、`client/dist`、前后端 `node_modules`、`server/.env`、全部 SQLite/WAL/历史快照/报告、日志/缓存、Docker/Nginx/部署文档、静态 `openapi.json`/`skill.md` 和 AyaNewsSkill 最新 main。初版归档 82 MB、29,449 个条目，SHA256 为 `9266c88bcc3f22e985a01e55e30837e6ea779ffe74c8f1129a7dfd70615b746b`；已由 Task 11 的新版完整包取代。归档权限为 `0600`，因含密钥与用户数据不上传公开 GitHub；临时明文 staging 已移入本机废纸篓，可恢复。
- 2026-08-31：完成 Task 11 HTTPS 冒烟遗留修复。确认 `/api/user-data/me` 匿名 `401` 与 `/api/admin/*` 无管理密钥拒绝均是预期安全边界；真实故障仅为 `/api/news/recommendations` 调用不存在的 `NewsService.getRecommendations`。按 RED→GREEN 新增旧数组契约兼容方法、1–30 `limit` 校验和 OpenAPI 路由。以 10,620 条 News 的本地生产候选库实测 HTTP 200 返回 10/10 条真实原链，来自 10 个不同来源并覆盖 `global/cn`，全部带推荐理由；非法 limit 为 400。最终 Server `348/348`、Client `48/48`、Vite 构建和 `git diff --check` 通过。[AiNews PR #6](https://github.com/MarcusDog/AiNews/pull/6) 已合并，远端 `main` 为 `83faeb0f9f2a38f65ee343f6d531e38aae16053e`。随后重建的完整离线包为 86,033,656 bytes、29,453 个条目、Git 元数据 0、关键路径 11/11，SHA256 `45a1fa71d363da4cc9a29455852b67007bacfb04d81f6933b8cec3974a53da41`；gzip、清单与最终文件 checksum 均通过。生产 HTTPS 仍需服务器凭据完成直接上传后复验。
...
- 2026-08-29：完成 Slice C Task 14 RED→GREEN。新增 preview-first retention、SQLite online backup、JSONL export、管理 API 与 CLI。预览将每张允许表的 cutoff 与最大 rowid 持久冻结，执行 token 绑定操作者、单次使用且过期拒绝；preview/execute/rejected/failed/backup/export 均写脱敏审计。Bridge payload 级联只删关联行，不删帖子/run；事件先于 outbox 清理，防止执行中候选集合扩大；72 小时外的 metrics/topic snapshot 压缩为每实体每天最新一条，180 天外删除。备份限定目录、拒绝 traversal/覆盖、失败只清理由本次创建的文件并做只读 integrity check；JSONL 由一致性事务输出 creators/accounts/posts/metrics/topics/evidence、schema/time-range/SHA256，明确不含 cursor、secret reference、Cookie/Authorization，异常不留临时文件。Task 14 定向 `24/24`、服务端全量 `312/312`，真实 CLI 空库 preview 返回 10 类全零候选。
