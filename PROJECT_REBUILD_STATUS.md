...
| Q. Slice A 可信采集与历史回填 | 进行中 | Task 0–1 已完成：基线保护测试 `5/5`；CreatorStore 已建立 23 张独立 Creator 表、7 个查询索引、WAL/外键、稳定账号/帖子唯一性、事务化帖子/指标/游标写入、回填与 Bridge payload 级联边界。Task 1 定向 `8/8`，含 legacy 回归 `27/27` 通过。Task 2 待开始。 |
