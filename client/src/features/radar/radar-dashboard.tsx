    ...
    <section id="radar" aria-labelledby="radar-title" className="radar-workspace relative z-20 bg-[#031a26] px-5 pb-24 pt-20 text-white sm:px-8 sm:pb-32 sm:pt-28">
      <div className="mx-auto max-w-7xl">
        ...
        {status === 'ready' && data && <div className="space-y-28">
          ...
          <section><SectionHeading eyebrow="creator opportunity" title="今日可做选题" note="选题角度只来自 Topic 标题、证据类型与可复现能力；单源主题会明确扣分。" /><div className="grid gap-x-12 gap-y-8 md:grid-cols-2">{data.topics.slice().sort((a, b) => b.creatorScore - a.creatorScore).slice(0, 4).map((topic) => { const angle = topic.opportunity.angles.find((item) => item.audience === 'creator') || topic.opportunity.angles[0]; return <div key={topic.id} className="border-t border-white/10 pt-6"><span className="font-display text-4xl">{topic.creatorScore}</span><span className="ml-2 text-[10px] uppercase tracking-[0.14em] text-white/35">creator score</span><h3 className="mt-4 text-lg font-medium">{angle?.title || topic.title}</h3><p className="mt-2 text-sm leading-6 text-white/48">{angle?.angle || topic.summary}</p><a className="mt-4 inline-flex text-xs text-white/60 hover:text-white" href={`/research?topic=${encodeURIComponent(topic.title)}&topicId=${encodeURIComponent(topic.id)}`}>研究这个选题 →</a></div>})}</div></section>
